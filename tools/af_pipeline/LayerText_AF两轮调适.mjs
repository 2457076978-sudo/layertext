...
const TIERS = {
  A: {
    key: 'A', label: 'A层（挑战）', clsTag: 'A层85', ratioRef: 0.85,
    goal: '独立读通，保留少量原文表达',
    words: '熟词优先；必要难词少量保留（每段最多 2 个加注词）',
    syntax: '简单句为主',
    reference: '有歧义就补出人物名字',
    anno: '支持少量必要词',
  },
  M: {
    key: 'M', label: 'M层（中层）', clsTag: 'M层75', ratioRef: 0.75,
    goal: '更直接、少推断',
    words: '非必要难词原则上替换（词汇尽量全落在课标内，每段最多 1 个加注词）',
    syntax: '拆开嵌套关系',
    reference: '人物切换时明确名字',
    anno: '更少引入新词；允许补解释',
  },
  B: {
    key: 'B', label: 'B层（基础）', clsTag: 'B层60', ratioRef: 0.6,
    goal: '明确人物、动作和因果',
    words: '尽量用核心常用词',
    syntax: '一句主要表达一件事',
    reference: '避免连续多句依靠代词',
    anno: '必要概念可集中预教；允许比 M 层更长',
  },
};
...
const UNIT_LIB = (() => {
  ...
  try {
    ...
    for (const b of bookOrder) for (const [u, v] of Object.entries(j.books[b])) {
      if (u === '（未标单元）') continue;
...
async function round1(i, t) {
  ...
  let done = [];
  if (existsSync(pf)) {
    try { done = JSON.parse(readFileSync(pf, 'utf-8')).done ?? []; } catch { done = []; }
  }
  ...
  for (let k = 0; k < segs.length; k++) {
    ...
    const revised = cleanSeg(await callChat([{ role: 'system', content: system }, { role: 'user', content: user }]), marker);
    out[k] = revised;
    done.push(k);
...
function localCheck(t, i) {
  ...
  if (!existsSync(r1)) { console.error(`✗ ${ch} 还没有初稿（先跑第一轮）`); process.exit(2); }
...
async function round2(t, i, feedbackRaw) {
  ...
  if (fb.magnitude && prog && UNIT_LIB) {
    ...
    for (let i2 = newIdx + 1; i2 <= idx; i2++) for (const w of UNIT_LIB.ladder[i2].words) {
...
function writeReport(t, r) {
  ...
  if (r.fb) {
    L.push('', '## 教师反馈与折算', `- 反馈原话：「${r.fb.raw}」`, `- 解析：处理维度 ${r.fb.dims.join('/') || '（未识别，按原话整体参考）'}；保留维度 ${r.fb.keep.join('/') || '—'}；幅度 ${r.fb.magnitude ?? '—'}`);

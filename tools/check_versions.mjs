 ...
 * package.json / app/package.json / tauri.conf.json / README「当前状态」四处版本必须一致，漂移即退出码 1。
 * 由 CI 调用（node tools/check_versions.mjs）；发版只改一处而漏其余的事故（README 滞留 v1.0.0）由本门禁终结。
 */
...
if (bad.length) {
  ...
  console.error('  发版时五处一起改：根 package.json、app/package.json、tauri.conf.json、README、CHANGELOG。');
  process.exit(1);
}
console.log(`\n✓ 版本四处一致：${pkg}`);

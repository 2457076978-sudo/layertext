...
const mdPath = argv[0], csvPath = argv[1];
if (!mdPath || !csvPath) {
...
const family = (w) => [w, w + 's', w + 'es', w + 'ed', w + 'ing',
  w.replace(/([^aeiou])e$/, '$1') + 'ing', w + w.at(-1) + 'ing', w + w.at(-1) + 'ed'];
...
if (absent.length) {
  ...
  console.log('---');
  console.log(`【复现词约束】以下 ${absent.length} 个词是学生上一章刚学过的已学词，请在改写中让每个词自然出现约 ${each} 次（可用其词形变化，如过去式/-ing），融入情节与对话，不得为了塞词改写情节：${absent.map((a) => a.word).join(' / ')}`);
  console.log('---');
}

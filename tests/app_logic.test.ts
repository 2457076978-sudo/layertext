...
test('★ 撤销之后必须**攒得下重做**：先在的稿进 redo，undo 退一格', () => {
  /* 这条守的是一个真发生过的缺陷：`persistEdit` 每次都会 `redoStack = []`，
   * 而 `doUndo` 原先在调它**之前**就把当前稿压进了 redo——压完立刻被清空，
   * 于是**重做永远没得做**，同时当前稿又被塞回 undo，撤销退化成"来回切"。 */
...
test('撤销/重做：空栈返回 null（调用方据此提示"没有可撤销的更改"）', () => {

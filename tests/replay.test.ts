...
/**
 * 回放夹具**不进公开仓库**（2026-09-13）：它里面是 Orwell《Animal Farm》原文
 * 与教师的三层改写产物、词库、词典、专名表、知识库——那是教师自己的材料，
 * 不该跟着 source-available 的公开仓库出去（那份文件头自己就写着"原文合法纳入后须逐段复核"）。
 *
 * 现在夹具放在**仓库外**，用 `LAYERTEXT_REPLAY_DIR=<夹具目录>` 指过来；
 * 没设或目录不在，这一层**整层跳过**（而不是假装通过，也不是把守卫悄悄降级）。
 * 本机默认位置：`~/Documents/LayerText配置/回放夹具_真项目`。
 */
const FIXTURE = (() => {
...
const NO_FIXTURE = HAS_FIXTURE ? false : '真项目回放夹具不在（设 LAYERTEXT_REPLAY_DIR=<夹具目录>）——夹具含 Orwell 原文与教师产物，不进公开仓库';
/** 一层用完就跑不了的守卫：夹具不在时，这一层整体 skip（测试名照常出现在报告里） */
const t = (name: string, fn: () => void | Promise<void>): void => {
  void test(name, { skip: NO_FIXTURE }, fn);
};
...
t('★ 冻结的结论能逐项复现（引擎层 · 全书每一章 × 三档）', () => {
  /* 这一条守的是**全书**：10 章 × A/M/B，每章自己的段级规则命中、质检、两条轴都要重算对上。
   * 只守第一章的版本里，第七/八/九章掉到 2%/3%/1% 谁也不知道——
   * "守一个我们愿意看的样本"和"不守"在事故面前没有区别。 */
  const r = spawnSync(process.execPath, [join(AF, 'LayerText_AF冻结回放.mjs'), '--check', '--root', FIXTURE], {
    cwd: REPO,
    encoding: 'utf-8',
    env: ENV,
    timeout: 300_000,
...
t('★ 端到端回放：**第七章**（当年掉到 2% 的那一章）也要跑出冻结的那一份', () => {
...
t('★ 端到端回放：任务组压缩比不回归（70 条 → ≤25 组）', async () => {
  const { root } = materialize();
  try {
    const q = runQueue(root, 'A', '1');
...
t('★ 覆盖不许缩水：每一章 × 每一层要么有结论、要么在缺失清单里（棘轮）', () => {
...
t('★ 事故章有单独的守卫：A 层第七/八/九章的加注覆盖率不许掉回个位数', () => {
...
t('★ 每一章都记着"本该加多少"：分子与分母成对出现，覆盖率不许低于 80%', () => {
...
t('★ README 里报给用户的数，必须与可复现的冻样本一致', () => {
  ...
  assert.equal(readme.includes('LAYERTEXT_REPLAY_DIR'), true, 'README 要说清这几个数**怎么复现**（含夹具从哪来），否则读者只能选择相信');
});

/* ────────────────────── ⑤ 反证：守卫真的会响 ────────────────────── */

t('★ 反证：抹掉第七章的注，对账必须当场失败（守的是全书，不是第一章）', () => {
...
t('★ partial 章显式声明：进分章与覆盖、不进全书分母、点名排除（第七轮 P2）', () => {
...
t('★ 全书端到端（慢测）：三档风险队列都要盖到每一章，一章都不许消失', () => {
  if (!FULL) return; // 慢测：设 LAYERTEXT_REPLAY_FULL=1 才跑（三档各跑一遍全书十章，比常驻子集慢一个量级）

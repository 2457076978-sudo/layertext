...
## 提供 5 个工具（全部本地计算、零遥测、不落盘）

| 工具 | 用途 | 典型场景 |
|---|---|---|
| `layer_qc` | 全文体检：覆盖率/生词率/句长/被动/定从/过去完成/OOV 清单（逐词带 zipf 词频分诊：高频未收=疑似漏收候选，低频=真·生词） | 简化前评估原文难度、简化后验收 |
...
OOV zipf 分诊（wordfreq 词频先验，`assets/wordfreq/en_zipf.tsv`，纯离线）：zipf ≥ 4.0 的高频词未进词库 → 标"疑似漏收"（教师核对后入库，勾选才生效）；3.0–4.0 中频；< 3.0 低频=真·生词教学优先。**只出候选，不碰判定**——"学过没有"的判定锚永远是词库表。表文件缺失时自动跳过分诊字段，不影响体检。

...
## [未发布] - 2026-09-16（架构地图 WordNet 双定位补正：清洗线在职）

**Wayne 指出 58e2c60 的备注片面**：只写了仓内视角（"全仓消费点仅词面板「英」"），
漏了清洗线在借用资产——材料词汇提取器是仓外脚本，消费 `app/public/wordnet` 索引做
真词保护网（v5.3 实证：点名逃逸 10/10 灭、38 探针 0 死，词频表 v1 即其产物）。
**补正为双定位**：① App 运行时＝冻结（维持 58e2c60 原判）；② 清洗线＝在职的真词公证人，
材料入库前清洗时调用——词频表不可替代（产物不能当网、量级差一个数量级、派生词推不出）。
**调用时机约定（Wayne 拍板"清洗材料的时候再调用"）**：只在清洗时调用，不进判定/换词链；
后期清洗工具 MCP 化后，AI agent 在清洗步骤直接调用。sensematch 维持全冻结（清洗线只用
wordnet 词表索引，不用消歧）。

## [未发布] - 2026-09-16（WordNet/义项消歧体系冻结 + push 后 CI 修复）

**架构拍板（Wayne）**：换词链事实 = 判定规则主导 + 换词 AI 主导；WordNet 消歧体系
（wordnet.ts/sensematch.ts + 8.2MB 资产）不在换词/判定/中文加注任何主链上，
全仓消费点仅 App 词面板「英」与工序化生成备用线，**AF 学生材料英英注实测 0 个**——装机未投产。
**冻结**：不砍（不可逆，砍要赌"永远不用"）、不补测（给不承重物花钱）；架构地图 4.x 标注
"非关键路径·未投产"。重新评估触发条件：英英注释产品化 / 论文写作需要 / ③顺位原计划取消。

**push + CI 修复**：阶段 A 后积压的 11 个提交推送；CI 红（构建+全量测试）→ 本地 worktree+npm ci
干净复现：wordsimpl 动态 import 把 app 模块拉进根 tsc 编译图，干净环境无 xlsx/docx → TS2307
（本机绿系 node_modules 历史残留掩盖）。修法：`tests/app-pkg-stubs.d.ts` 类型层桩（class 声明值型两用、
Packer→Buffer、WorkBook 泛型），与运行时导入占位配套，零新依赖、audit 保持 0。
**CI run 74 绿**。两笔修复提交曾在 verify 未绿时入库（3a3500a/6be0c92），已在提交信息自纠记录。


## [未发布] - 2026-09-16（词频表防误用标注：exposure 口径警示入库）

AF 知识文件新增 `README_材料词频表.md`（与 材料词频表_v1.csv 同目录）：
明示 exposure 口径、**禁止直接并入已学/词库表**、词频高≠掌握、已定用途=分档取词依据。
防未来任何人（含 Agent）顺手并表的事故——本轮 zero-code 防护。


## [未发布] - 2026-09-16（材料词频表入库 v1：AF 知识文件新增 材料词频表_v1.csv）

Wayne 拍板"把词频表放进知识库，去掉人名噪音"。`知识文件/材料词频表_v1.csv` = **11202 词**
（11324 再剔 122：英文名 64、拼音姓名 38、地名/品牌 13、噪音缩写 7——sth/tv/mr 类；
jack/rose/mark/sun/may 等词名两读由课标表保护保留）。审计：`材料词汇提取_2026-09-16/已剔除人名噪音.csv`。
再次在案：本表是 **exposure 口径**（材料里出现过 ≠ 已学会），供选词/难度参照；
是否并入任何"已学/词库"表仍是未决产品决策。


## [未发布] - 2026-09-16（材料词汇提取与清洗 v5.3：教学材料→11324 词干净底稿）

**产物在仓外**（AF 工作区 `知识文件/材料词汇提取_2026-09-16/`），本仓只记日志与交叉依赖——
提取器消费了本仓 `app/public/wordnet/index.*.gz`（真词保护网，见下）。

**提取**：Downloads 全部英语教学材料（1022 份：九上 U1-U3 课件、人教 789 试题库、
15-25 贵州中考卷、月考/期末卷、扫描件 4 份 Swift Vision OCR）只读提取 → 原始 16358 词。

**清洗五轮（探针驱动，误杀→修复全程留痕于提取报告.md）**：
- v2：乱码/答案串/残片剔除（词频≤4 门槛）→ 14557；
- v3：撤频率门槛**大面积误杀**（drink/spent/dress/families/playground）——审计探针抓住；
- v4：引入 **WordNet 原形索引 14.7 万词做真词保护网**（课标/教材 ∪ WordNet ∪ 剥变形原形 ∪
  连字符变体），固定顺序 乱码→残片→归并；后续小修：残片加"基础形词频≥残片×2"绝对比
  （kate 不再被当 skate 截断）、撇号形归并表前置（didnt→did）、连写规则撤销（误杀 pingpong 类
  复合词）、单元音规则加词频<10 门槛（slept/drank/threw 不规则过去式存活）；
- 终态 v5.3：**11324 词**，高频逃逸（eed/wcxkfmgng 类）10/10 灭、38 个真词探针 0 死亡、
  词频≥5 剔除 256 个逐一复核全为真垃圾。

**变形归并基础形（Wayne 指令"变形只留一个基础形"）**：rays→ray、connecting→connect、
families→family、tshirts→tshirt 共 **3364 条**，审计在 `已归并变形词.csv`。

**口径提醒（当日五问已答，记录在案）**：本表统计的是 **exposure（出现过）**，主体来自试卷与
练习（非教师自己的课件）；与"已学口径"（教学计划覆盖/成员判定）是两套语义，**消费路径未决**——
并入哪张表（词库/注释词典/已学词）等 Wayne 拍板，未动任何知识库文件。


## [未发布] - 2026-09-16（核心链补测 ②：loadTextbookLearned 全分支 + 真实锚点；教材进度启用）

- 真实项目启用教材进度 **九上:U3**（调适项目_AnimalFarm.json；原值 null=链路关闭）——
  真实口径下已学词集 **3428** 个（base1614 + 八上/八下全册 + 九上 U1-U3）。
- 五条 case：未配置→null / 前册全计·后续不计·未标单元不计 / half=ceil(半) /
  缺册静默跳过（Wayne 拍板"跳过"，合成库刻意缺七上七下）/ 真实库锚点 3428（库更新时应有意更新）。
- 另：教学材料词汇提取（Downloads 1022 份→词汇 16358）产物在 AF 工作区
  知识文件/材料词汇提取_2026-09-16/（不属本仓，CHANGELOG 仅记交叉引用）。

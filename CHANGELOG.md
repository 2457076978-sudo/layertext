...
## [未发布] - 2026-09-13（codex/total-optimization · 排版重构（方案 B）：四条横条 → 两行 + 左侧竖栏，172px → 88px）

> 来源：Wayne 截图反馈"这里菜单栏太急了，我想要完全重构做一个好看好用的排版"，并选定**方案 B**
> （两行 + 左侧竖栏）。设计对比与三个方案的说明见桌面 `UI重构_排版方案_2026-09-13.png`。
...
模式胶囊/状态文字）。挤到最后状态文字只剩 **128px**，Wayne 截图里那个 `已进入【A层重制·…`
就是被挤断的——**它想说的信息等于没了**。另有两处浪费：版本三个胶囊常驻一整行 38px（一天切两三次），
章节标签只放一个文件时也占一整行。

**核心思路：按"这个动作属于哪一级"分层，每级只出现一次。** 落地成两行 + 一栏：

```
顶栏 48px   ●●● LayerText │ 书架 打开…        [↩ ↪ 🔍 A－ A＋ ☀ ⚙] [候选模式]  状态（吃满余量）
上下文栏 40  [版本 A层重制·原文85% ⌄] │ [第一章 ×][第二章] …    [质检本章][按标记修改][学生视角]
────────────┬──────────────────────────────────────────────┬────────
 竖栏 56px  │  正文 ｜ 逐句对照 ｜ 版本对比                    │  审校栏
 目录       │  ┌────────────────────────────────────────┐  │
 读检改库   │  │  正文…                                  │  │
────────────┴──────────────────────────────────────────────┴────────
```

- **一级导航竖起来**（目录 + 读/检/改/库 → 左侧 56px 栏，各配图标）：横排那条既吃 44px 高度、
  又把章节标签挤窄；竖起来之后**高度省掉一整条，横向宽度整条让给章节标签**，一级（栏）/二级（条）
  的分工也一眼看得清。二级页签贴到正文列顶部，改成下划线式；**当前组只有一个视图（库）时整条收起**
  ——只有一项的导航条是噪音。
...
**踩到并修掉的坑（记在 CSS 注释里）**：竖栏 `display:none` 之后网格会**自动补位**，正文被塞进
56px 的第一列（书架页实测，正文只剩 24px 宽）——所以 `body.no-session` 必须把栅格改成单列
（书架页连侧栏也收起）。这类"藏一个格子 ≠ 少一列"的账，只改 `display` 是还不清的。

**没动的**：正文渲染、审校面板、右侧栏、所有交互逻辑一行没改；只动 `header/ctxbar/rail/subtabs`
这一层（`app/index.html`、`archive.css`、`widgets.ts`、`main.ts`、`shelf.ts`）。

**验收**：`tsc` 双工程 + `eslint --max-warnings 0` + `preflight` + `vite build` 通过；
全量测试 **955 项 954 通过 / 1 跳过 / 0 失败**。版式用无头 Chrome 在 1200×760/800/900 下、
书架态与 读/检/改/库 四组逐一截图核对（含版本下拉展开、库组二级条自动收起、书架页单列栅格）。

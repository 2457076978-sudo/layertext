...
**验证**：`npm run verify` 全绿。新增 `tests/bundle.test.ts`（18）· `tests/bundle_cli.test.ts`（6，真跑子进程）· `tests/pipeline_gate.test.ts` 增 6。另：阶段 2「任务工作台」（今日任务/暂停恢复/变更历史）由并行子代理落地，见同一批次的后续条目。

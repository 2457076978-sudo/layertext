...
A 层几乎全是"加支架"而不是"降负荷"，这个区别在合成指标里会被抹平。）

换一本书 / 失败后怎么续跑 / 报告里那些覆盖率各是什么：[换书与续跑说明](docs/换书与续跑说明.md) · 判定与人工队列：[段级门禁与风险队列](docs/段级门禁与风险队列.md) · 审查报告逐条落实与证据：[落实对照表](docs/审查报告_落实对照表.md)。

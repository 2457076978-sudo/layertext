...
mod dict_tests {
    ...
    fn local_dict_returns_zh_gloss() {
        ...
        //
        // 缺外部资源不是实现失败，所以默认跳过——但 **Rust 把"正常返回"记为通过**，
        // 于是没有系统词典的机器（Linux CI、新装的机器）上这条会**永远绿而没人知道**：
        // 与 TS 侧那条 `if (!existsSync(p)) return;` 是同一类假绿（那边已改成 t.skip）。
        // Rust 没有条件跳过机制，所以给一个**显式硬开关**：
        // `LAYERTEXT_REQUIRE_SYS_DICT=1` 时缺词典就当场失败；默认仍跳过，但会打印原因。
        let Some(zh) = crate::dict::lookup_zh("boar") else {
            assert_ne!(
                std::env::var("LAYERTEXT_REQUIRE_SYS_DICT").as_deref(),
                Ok("1"),
                "LAYERTEXT_REQUIRE_SYS_DICT=1 要求本机必须有系统词典，但 lookup_zh 返回了 None"
            );
            eprintln!("⚠ 跳过 local_dict_returns_zh_gloss：本机没有系统词典（要强制检查请设 LAYERTEXT_REQUIRE_SYS_DICT=1）");
            return;
        };

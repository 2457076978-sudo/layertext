...
export async function renderRiskPane(
   ...
   * 「⏸ 暂停/继续」「✓ 我做完了」以及「看我判过的」里的「↩︎ 撤销」——
   * 它们被渲染出来，却因为绑定循环在下面、被 return 跳过，**一个监听都没有**。
   * 而"这一层我判完了"恰恰是最想回头撤销一条、或记一笔收工的时刻：按钮就在眼前，点了纹丝不动。
   * 现在只记一个空态 HTML，让下面的绑定循环照常跑完（没有卡片时那几组 querySelectorAll 自然为空）。
   * 安全性：空态下 `groups` 是空数组，`card`/`groupHtml` 只是函数定义、不会被调用。 */

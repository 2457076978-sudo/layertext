...
export async function loadClassGroups(): Promise<void> {
  try {
     ...
     * 于是这个 `.endsWith('.json')` 过滤器**恒得空数组**：`S.classTargets` 恒为空、
     * `mergedSelection()` 恒 `active:false`，班级口径（句长上限/复现词并集/产物标签）
     * 整体不生效，面板只会说"未找到分组文件"。返回的是完整路径，下面按路径读，本来是对的。 */
...
export function toggleClsPanel(): void {
   ...
   * `#cls-panel` 由 `renderClsPanel()` 惰性创建（它自己 `createElement` + `appendChild`），
   * 而这里在调它**之前**就 `if (!p) return` —— 第一次点永远命不中，于是永远没有第一次创建。
   * 两个入口（设置弹层「选择班级…」、原生菜单「班级多人定制…」）走的都是这条死路，
   * 连带的关闭/刷新/清空与所有班级勾选框全部不可达。
   * 现在：没有就先让 `renderClsPanel()` 把它建出来（建出来时 display 是 none，下面按"打开"处理）。 */
...
export function showAiSettings(): void {
  ...
  const addFbRow = (name = '', url = '', model = '', key = '', id = '') => {
     ...
     * 新建行当场生成一个，已存在的行沿用配置里的那个——**不再用行下标**。 */
...
function renderSettings(): void {
  ...
  document.getElementById('set-autorew')?.addEventListener('change', (e) => {
    ...
    void saveConfig();
    /* 2026-09-14：这里原先不刷顶部模式胶囊——于是在设置里改完、关掉设置，
     * 胶囊仍显示旧模式，而 `reader.ts` 读的是新值：**界面说的和程序做的相反**。
     * 同文件保存那一路（renderSettings 之外的 $('ai-save')）是刷的，这里漏了。 */
    updateModePill();
    toast('已保存');
  });
}

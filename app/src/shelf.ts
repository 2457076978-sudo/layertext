...
function renderShelfGrid(el: HTMLElement, books: ShelfBook[], covers: Map<string, string | null>): void {
  ...
  /* 2026-09-14 修（**死按钮**）：`#shelf-demo`（内置示例）与 `#shelf-add`（添加书稿文件夹）
   * 的 DOM 由**本函数**重建（见上方模板），而它们的监听原先只在 `bindShelfChrome` 里绑过一次。
   * 于是：在搜索框敲一个字、点「进度/书封」切视图、点任意分组 chip —— 这三条都会重跑本函数，
   * 之后这两个按钮**再没有任何监听**：不弹文件框、不报错、不 toast，纯粹点了没反应，
   * 只有回到书架首页才恢复。改成跟着这次重建一起绑（`bindShelfChrome` 里相应地删掉，
   * 否则首次渲染会被绑两遍）。 */
 ...
 *  （示例与添加两个按钮的 DOM 由 `renderShelfGrid` 重建，绑定也在那边——见该函数末尾的注释） */
...
export function renderWorkspaceBar(): void {
  ...
  el.innerHTML =
    S.workspaces
      ...
      .join('') +
    (active
      ? `<label class="ws-files">章节 <select aria-label="切换工作区章节" data-ws-select>${active.文件
          .map(
            (f, i) =>
              `<option value="${esc(f)}" ${f === cur ? 'selected' : ''}>${esc(
                f
                  .split('/')
                  .slice(0, -1)
                  .reverse()

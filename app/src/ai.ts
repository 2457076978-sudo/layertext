...
/* 2026-09-14：这三个此前**没被登记**，而 `prompts/` 里有文件、`manifest.json` 里也有条目。
 * 少了它们，`loadPrompt` 拿到的是 `BUNDLED_PROMPTS[name] ?? ''` = **空串**，
 * 于是「AI 批改建议」「读后检测题」「定向复习材料」三条链路都在发**空提示词**——
 * 白花一次 API 调用，教师拿到的还是模型自由发挥的产物。
 * （旁证：`app/dist/` 里 grep `prompts/grading.md` 的首行「正在批改学生的英语写作」零命中。） */
...
async function activeTargets(): Promise<ProviderTarget[]> {
  ...
  for (let i = 0; i < (cfg.failover ?? []).length; i++) {
    ...
    try {
       ...
       * 关键在这一句的**条件**：只有**没有 id**（老配置）才回落下标账号。
       * 原先写的是"稳定账号取不到就回落下标"——那让「清」按钮变成假动作：
       * 删掉 `fb:<id>` 之后立刻从 `fb0` 把同一把 Key 读了回来，界面上说"已删除"，
       * 实际照旧在用；更糟的是删过行以后 `fb0` 已经属于别家，会串 Key。
       * 老 Key 怎么搬：在 `settings.ts` 给一行**新分配稳定 id 的那一刻**搬一次
       * （那时下标还有意义）。这里刻意**不做兜底迁移**——那会把教师刚「清」掉的 Key 又搬回来。 */

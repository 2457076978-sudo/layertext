...
export async function applyWordSimplifications(s: FileSession, marks: Mark[]): Promise<void> {
  ...
  for (const m of uniq) {
    ...
    for (let i = hits.length - 1; i >= 0; i--) {
      ...
      /* 2026-09-14：基准必须是**不变的** replBase。原先 `let r = repl`——而 repl 会在首次迭代
       * （倒序循环＝最右侧那处）末尾被写成带首字母大写的形态，于是它左侧所有**小写**出现处
       * 都被替换成大写词并落盘。 */
      let r = replBase;
  ...
  void propagateToLowerTiers(
    s,
    simplified.map((p) => ({ word: p.word, op: 'rewrite' as const, zh: p.zh })),
  );

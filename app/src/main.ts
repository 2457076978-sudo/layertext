...
import { renderSidebar, scheduleSave, setMarkSaveErrorReporter } from './review.js';
...
export async function addSession(md: string, fileName: string, sourcePath: string | null, opts: { noAutoQc?: boolean } = {}): Promise<void> {
  ...
  if (saved !== null) {
     ...
     * 并且不把损坏文件的内容当成空表写回去（`markFileBroken` 会拦住保存）。 */
    try {
...
function closeSession(i: number): void {
  ...
  /* 2026-09-14：原先只有 `Math.min(activeIdx, len-1)`——关掉**当前标签左边**的标签时，
   * 数组左移一格而 activeIdx 不动，界面会悄悄切到另一个章节：
   * 之后的标记 / AI 改写 / 撤销全作用在**错误的文档**上，且没有任何提示。
   * 关掉的正是当前那一个时才需要封顶。 */
 ...
 * 写盘并**记一次编辑历史**。
 *
 * `opts.recordHistory === false` 是给「撤销/重做」用的：那两个动作自己管理
 * undo/redo 两个栈，**不能让这里再记一遍**——否则 `redoStack = []` 会把刚压进去的重做项清掉
 * （2026-09-14 之前正是如此：`doUndo` 压了 redo，紧接着 `persistEdit` 把它清空，
 * 于是**重做永远没得可做**；同时 `undoStack` 又被塞回当前稿，撤销变成"来回切"）。
 */
